...
> **选题 / 编号规则**
> - **跨类别轮询（硬规则，别再盯着一个类别写）**：每次在「本类别下还有 slug 未做」的类别里，挑**已发布篇数最少**的那个类别（并列时按本文件从上到下的类别 `##` 顺序取靠前者）；再在该类别内挑**最靠上**的未做 slug。目的是让连续几篇雨露均沾、来自不同类别——尤其别把「深度学习 / 现代 AI」一路写下去（它已铺得很厚）。
>   - 判定：`ls *-paper*.html`（去掉 `.en` 去重）得已发布 slug 集；某 slug 属于哪个类别，看它在本文件哪个 `##` 标题下。两个「深度学习 / 现代 AI」子簇各自算一个类别（AI 已很多，「最少」规则会让它短期内自然不被选中）。
> - 文件名 `paper{N}` 的 **N 用写作顺序的下一个编号**（现有最大 N + 1），**与类别 / 清单顺序解耦**——轮询顺序不影响文件编号。
> - 这份清单**人工策划、routine 只读不改**（`publish.sh` 硬卡 TOPICS.md 改动）。全部类别都做完 → PushNotification 请 BigCat 续单、本次不发布。
