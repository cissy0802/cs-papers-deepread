...
## Google 分布式系统（三件套与演进）
- **GFS** — The Google File System · Ghemawat 等 · 2003 — 廉价机器 + 副本容错撑起 PB 级分布式存储（slug: `google-file-system`）
- **MapReduce** — Simplified Data Processing on Large Clusters · Dean & Ghemawat · 2004 — 把大规模并行抽象成 map+reduce，开启大数据时代（slug: `map-reduce`）
- **Bigtable** — A Distributed Storage System for Structured Data · Chang 等 · 2006 — 稀疏多维有序宽表，列式存储先驱（slug: `bigtable`）
